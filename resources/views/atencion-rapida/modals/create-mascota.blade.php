@php
    ...
    $mascotaStoreErrors = $errors->atencionRapidaMascotaStore;
    $hasMascotaStoreErrors = $mascotaStoreErrors->any();
@endphp

<div id="atencionRapidaMascotaModal"
    ...
    <div class="modal-card flex max-h-[calc(100vh-3rem)] w-full max-w-3xl scale-95 flex-col overflow-hidden rounded-[28px] border border-slate-200 bg-white opacity-0 shadow-2xl transition-all duration-200 ease-out">
        <div class="shrink-0 border-b border-slate-100 bg-white px-6 py-5">
            <div class="flex items-start justify-between gap-4">
                <div>
                    <p class="text-sm font-medium text-emerald-600">Atencion rapida</p>
                    <h2 class="mt-1 text-2xl font-bold text-slate-900">Nueva mascota</h2>
                    <p id="atencionRapidaMascotaSummary" class="mt-1 text-sm text-slate-500">Registra a la mascota para continuar con la atención sin cita.</p>
                </div>

                <button type="button"
                        onclick="closeAtencionRapidaMascotaModal()"
        ...
        <form id="atencionRapidaMascotaForm"
              method="POST"
              action="{{ route('atencion-rapida.mascotas.store') }}"
              enctype="multipart/form-data"
              class="min-h-0 flex-1 space-y-5 overflow-y-auto px-6 py-6">
            @csrf
            <input type="hidden" name="raza" id="atencion_rapida_modal_raza" value="{{ old('raza') }}" data-current="{{ old('raza') }}">
            ...
            <div class="grid gap-5 md:grid-cols-2">
                <div class="md:col-span-2 rounded-2xl border border-emerald-100 bg-emerald-50 px-4 py-3 text-sm text-emerald-800">
                    Crea a la mascota y el sistema la dejará lista para seleccionarla en la atención sin cita.
                </div>

                <div class="md:col-span-2">
                    <label class="mb-2 block text-sm font-semibold text-slate-600">Cliente o due&ntilde;o</label>
                    <input id="atencion_rapida_modal_cliente_search" type="search" placeholder="Buscar por nombre o DNI..." class="mb-3 w-full rounded-2xl border border-emerald-100 bg-emerald-50/60 px-4 py-3 text-sm text-slate-700 placeholder:text-slate-400 shadow-sm focus:border-emerald-500 focus:bg-white focus:ring-4 focus:ring-emerald-100">
                    <select name="cliente_id" id="atencion_rapida_modal_cliente_id" class="w-full rounded-2xl border {{ $mascotaStoreErrors->has('cliente_id') ? 'border-red-500' : 'border-slate-200' }} px-4 py-3 text-sm text-slate-700 shadow-sm focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100">
                ...
                <div>
                    <label for="atencion_rapida_modal_tipo_animal" class="mb-2 block text-sm font-semibold text-slate-600">Tipo de animal</label>
                    <select id="atencion_rapida_modal_tipo_animal" name="tipo_animal" class="w-full rounded-2xl border {{ $mascotaStoreErrors->has('tipo_animal') ? 'border-red-500' : 'border-slate-200' }} px-4 py-3 text-sm text-slate-700 shadow-sm focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100">
                ...
                <div>
                    <label for="atencion_rapida_modal_raza_select" class="mb-2 block text-sm font-semibold text-slate-600">Raza</label>
                    <select id="atencion_rapida_modal_raza_select" class="w-full rounded-2xl border {{ $mascotaStoreErrors->has('raza') ? 'border-red-500' : 'border-slate-200' }} px-4 py-3 text-sm text-slate-700 shadow-sm focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100">
                ...
                <div id="atencion_rapida_modal_input_otro_raza" class="hidden md:col-span-2">
                    <label for="atencion_rapida_modal_raza_otro" class="mb-2 block text-sm font-semibold text-slate-600">Especifica la raza</label>
                    <input type="text" id="atencion_rapida_modal_raza_otro" maxlength="255" class="w-full rounded-2xl border border-slate-200 px-4 py-3 text-sm text-slate-700 shadow-sm focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100" placeholder="Escribe la raza">
                </div>
            ...
            <div class="flex flex-col gap-3 border-t border-slate-100 pt-5 sm:flex-row sm:justify-end">
                <button type="button" onclick="closeAtencionRapidaMascotaModal()" class="rounded-2xl border border-slate-200 px-5 py-3 text-sm font-semibold text-slate-600 transition hover:border-slate-300 hover:bg-slate-50">
                    Cancelar
                </button>
...
@if ($hasMascotaStoreErrors)
    <script>
        window.atencionRapidaMascotaModalState = {
            hasErrors: true,
